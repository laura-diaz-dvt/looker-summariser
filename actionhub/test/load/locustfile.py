...
class ActionHubTaskSet(TaskSet):
    ...
    def get_something(self):
        ...
        data = {
            ...
            "form_params": {
                "sleep": 0,
                "simulated_download_url": os.getenv(
                    "ACTION_HUB_LOAD_TESTING_SIMULATED_DOWNLOAD_URL"
                ),
            },
        }
        ...
        headers["Authorization"] = (
            'Token token="' + os.getenv("ACTION_HUB_LOAD_TESTING_API_KEY") + '"'
        )

        with self.client.post(
            uri, catch_response=True, json=data, headers=headers
        ) as response:
